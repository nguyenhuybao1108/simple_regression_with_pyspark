...
class classification:
    ...
    def fit_transform(self):
        ...
        model = DecisionTreeClassifier(featuresCol="features", labelCol=self.label)

        return model

    def cross_validation(self):
        ##=====build cross valiation model======
        dt = self.fit_transform()
        # parameter grid
        ...
        AUC_training = evaluator.evaluate(pred_training_cv)
        AUC_test = evaluator.evaluate(pred_test_cv)
        print(f"{AUC_training=}")
        print(f"{AUC_test=}")
        print(
            "Intercept: ",
